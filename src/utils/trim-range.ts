// Trim leading/trailing whitespace from a DOM Range before it is anchored.
//
// Ported from the Hypothesis client (client/src/annotator/anchoring/trim-range.ts).
// A triple-click selection commonly includes a trailing newline; locking that
// whitespace into the text-quote and structural offsets makes anchors fragile
// across reflows/surfaces. trimRange() tightens both boundaries to the nearest
// non-whitespace character (across node boundaries) without touching the
// anchoring math downstream.
 ...
 *
 * @param range - A DOM Range whose `startContainer` and `endContainer` are
